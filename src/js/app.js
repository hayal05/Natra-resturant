  ...
  document.querySelector('#app').innerHTML = `
    <main class="login">
      <section class="login-box">
        ...
        <h1>NATRA</h1>
        <p>Restaurant Management · First-time setup</p>
        <form id="setup">
          <label for="name">Full name</label>
          <input id="name" autocomplete="name" required>
          <label for="user">Username</label>
          <input id="user" minlength="3" autocomplete="username" required>
          <label for="pass">Password</label>
          <input id="pass" type="password" minlength="8" autocomplete="new-password" required>
...
async function dashboard() {
  let summary = {};
  try {
    summary = await call('dashboard_summary');
  } catch (error) {
    console.error(error);
  }
  ...
  document.querySelector('#app').innerHTML = `
    <div class="app">
      <aside class="sidebar">
        ...
        <nav class="nav">
          ${nav.map((item, index) => `<button class="${index ? '' : 'active'}" type="button">${item}</button>`).join('')}
        </nav>
      ...
      <main class="main">
        <header class="topbar">
          ...
          <span class="status">● Local mode</span>
        </header>
        <section class="cards">
        ...
        <section class="panel">
          ...
          <p>The authentication foundation is complete. Features will be added one module at a time.</p>
        </section>
      </main>
    </div>`;
